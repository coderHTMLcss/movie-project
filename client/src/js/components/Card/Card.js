...
export default class Card {
  ...
  render() {
    this.moviesList.forEach((movie) => {
      if (movie.trending === true) {
        ...
        movieLikeSvg.innerHTML = `<svg  class="svg-image1" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <svg  class="svg-image2" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  toggleSVG(movie, event) {
    ...
    const storageData = localStorage.getMovie();

    if (!storageData) {
      const movieList = [movie];
      localStorage.putMovie(movieList);
    } else {
      localStorage.putMovie([...storageData, movie]);
    }

    const svg1 = event.currentTarget.querySelector(".svg-image1");
    const svg2 = event.currentTarget.querySelector(".svg-image2");
...
// removeMovie(movieId) {
//     const movies = this.getMoviesFromStorage().filter(movie => movie.id !== movieId);
